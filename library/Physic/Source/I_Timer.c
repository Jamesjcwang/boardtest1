...
static void(*_timer_raiseevent)(enum_Timer_Mis eventkind,
                                enum_Timer_BaseAddress port);

static uint8_t _step_timer[Timerindex_total];
static bool _f_ready_timer[Timerindex_total];
...
__STATIC_INLINE void nrf_timer_int_disable(NRF_TIMER_Type * NRF_TIMERx, uint32_t timer_int)
{
 ...
 static void _TIMER_Configure(enum_Timer_BaseAddress port,
                              enum_Timer_Bitmode bitmode,
                              enum_Timer_Prescaler prescaler )
  {

      nrf_timer_frequency_set((NRF_TIMER_Type*)port,prescaler);
      nrf_timer_bit_width_set((NRF_TIMER_Type*)port,bitmode);
      nrf_timer_shorts_enable((NRF_TIMER_Type*)port,0x3F3F);

      switch (prescaler)
      {
      case Timer_FREQ_16MHz:
        Timer_frequency=16000000;
        break;
      ...
      case Timer_FREQ_2MHz:
        Timer_frequency=2000000;
        break;
      case Timer_FREQ_1MHz:

        Timer_frequency=1000000;
        break;
      case Timer_FREQ_500kHz:
        Timer_frequency=500000;
        break;
      case Timer_FREQ_250kHz:
        Timer_frequency=250000;
        break;
      case Timer_FREQ_125kHz:
        Timer_frequency=125000;
        break;
      case Timer_FREQ_62500Hz:
        Timer_frequency=62500;
        break;
      case Timer_FREQ_31250Hz:
        Timer_frequency=31250;
        break;
...
static void _TIMER_IntConfigure(enum_Timer_BaseAddress port, enum_Timer_Mis interrupt,
                           bool enable,uint32_t priority,bool controllerenable)
  {
     switch (port)
     {
     case TIMER0:
        Timer_intcontroller->Configure(TIMER0_IRQn ,
                                       controllerenable,priority);

     case TIMER1:
         Timer_intcontroller->Configure(TIMER1_IRQn ,
                                       controllerenable,priority);
     case TIMER2:
         Timer_intcontroller->Configure(TIMER2_IRQn ,
                                       controllerenable,priority);
     case TIMER3:
         Timer_intcontroller->Configure(TIMER3_IRQn ,
                                       controllerenable,priority);
     case TIMER4:
         Timer_intcontroller->Configure(TIMER4_IRQn ,
                                       controllerenable,priority);
     ...
     if (enable)
      {
          nrf_timer_int_enable((NRF_TIMER_Type*) port,interrupt);
      }
      else
      {
         nrf_timer_int_disable((NRF_TIMER_Type*)port,interrupt);
      }

  }


static void _TIMER_IntClear(enum_Timer_BaseAddress port,
                     enum_Timer_Mis interrupt)
  {
      switch (interrupt)
      {
      case Timer_COMPARE0:
        nrf_timer_event_clear((NRF_TIMER_Type*)port,
...
static uint32_t _TIMER_IntStatus(enum_Timer_BaseAddress port,enum_Timer_Mis interrupt)
  {
      switch (interrupt)
      {
      case Timer_COMPARE0:
        return (nrf_timer_event_check((NRF_TIMER_Type*)port,
                              NRF_TIMER_EVENT_COMPARE0));
  ...
  uint8_t* getsteptimer(enum_Timer_BaseAddress port)
 {
    switch (port)
      {
      case TIMER0:

       return(&_step_timer[index_Timer0]);

      case TIMER1:

       return(&_step_timer[index_Timer1]);

      case TIMER2:

       return(&_step_timer[index_Timer2]);
      case TIMER3:

       return(&_step_timer[index_Timer3]);
      case TIMER4:

       return(&_step_timer[index_Timer4]);

      }
     return(0);
 }

  bool* getflagtimer(enum_Timer_BaseAddress port)
 {
    switch (port)
      {
      case TIMER0:

       return(&_f_ready_timer[index_Timer0]);
      case TIMER1:

       return(&_f_ready_timer[index_Timer1]);
      case TIMER2:

       return(&_f_ready_timer[index_Timer2]);
      case TIMER3:

       return(&_f_ready_timer[index_Timer3]);
      case TIMER4:

       return(&_f_ready_timer[index_Timer4]);
      }
     return(0);
 }
 ...
 static bool _TIMER_Start(bool reset,enum_Timer_BaseAddress port,uint32_t period
                          ,uint32_t divide)
 {
     if (reset==true)
     {
      *getsteptimer(port)=0;
      *getflagtimer(port)=false;
     }

     switch (*getsteptimer(port))
     {
     case 0:

       nrf_timer_cc_write((NRF_TIMER_Type*)port,
                           NRF_TIMER_CC_CHANNEL0,
                           (Timer_frequency*period)>>divide);

       nrf_timer_task_trigger((NRF_TIMER_Type*)port,
                            NRF_TIMER_TASK_START);


      *getsteptimer(port)=10;
       return (false);
     case 10:

        if (*getflagtimer(port)==true)
        {
           *getsteptimer(port)=0;
           return (true);
        }
...
static void _TIMER_Stop(enum_Timer_BaseAddress port)
 {
      nrf_timer_task_trigger((NRF_TIMER_Type*)port,
                        NRF_TIMER_TASK_STOP);
 }
...
static void _TIMER_Clear(enum_Timer_BaseAddress port)
 {
      nrf_timer_task_trigger((NRF_TIMER_Type*)port,
   ...
   static void _TIMER_IntHandler(void(*Handler)
                                 (enum_Timer_Mis eventkind,
                                  enum_Timer_BaseAddress port) )
  {

         _timer_raiseevent=Handler;

  }

 static void _timer_handler(enum_Timer_BaseAddress port)
 {
     if (_TIMER_IntStatus(port,Timer_COMPARE0))
      {
          _TIMER_IntClear(port,Timer_COMPARE0);
          *getflagtimer(port)=true;

      }
    if (_TIMER_IntStatus(port,Timer_COMPARE1))
      {
          _TIMER_IntClear(port,Timer_COMPARE1);
           *getflagtimer(port)==true;
      }
    if (_TIMER_IntStatus(port,Timer_COMPARE2))
      {
          _TIMER_IntClear(port,Timer_COMPARE2);
          *getflagtimer(port)==true;
      }
    if (_TIMER_IntStatus(port,Timer_COMPARE3))
      {
          _TIMER_IntClear(port,Timer_COMPARE3);
           *getflagtimer(port)==true;
      }
    if (_TIMER_IntStatus(port,Timer_COMPARE4))
      {
          _TIMER_IntClear(port,Timer_COMPARE4);
          *getflagtimer(port)==true;
      }
 ...
 void TIMER0Handler(void)
  ...
  void TIMER1Handler(void)
  {
    ...
    _timer_handler(TIMER1);
  }
  void TIMER2Handler(void)
  {

    _timer_handler(TIMER2);
  }

  void TIMER3Handler(void)
  {
    _timer_handler(TIMER3);
  }

  void TIMER4Handler(void)
  {
    _timer_handler(TIMER4);
  }

 void CreateTimerDevice(TIMERDevice* device,
                        InterruptController* intcontroller,
                        enum_Timer_BaseAddress port,
                        enum_Timer_Bitmode bitmode,
                        enum_Timer_Prescaler prescaler)
 {
    device->TIMER_Clear=_TIMER_Clear;

    device->TIMER_IntClear=_TIMER_IntClear;
    device->TIMER_IntConfigure=_TIMER_IntConfigure;
    device->TIMER_IntStatus=_TIMER_IntStatus;
    device->TIMER_Start=_TIMER_Start;
    device->TIMER_Stop=_TIMER_Stop;

    Timer_intcontroller=intcontroller;
    Timer_intcontroller->IntHandler(TIMER0_IRQn,TIMER0Handler);
    Timer_intcontroller->IntHandler(TIMER1_IRQn,TIMER1Handler);
    Timer_intcontroller->IntHandler(TIMER2_IRQn,TIMER2Handler);
    Timer_intcontroller->IntHandler(TIMER3_IRQn,TIMER3Handler);
    Timer_intcontroller->IntHandler(TIMER4_IRQn,TIMER4Handler);
    _TIMER_Configure(port,bitmode,prescaler);
